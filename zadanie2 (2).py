...
mass = 1.989e30 #int kg
dt = 3600 #int sekundy
r0 = 149597870700*0.586 #int m
v0 = 54600 #int m/s
G = 6.6741e-11#int N⋅m2⋅kg−2
obroty = 3 #int na pewno działa do 10 obrotów większa ilość nie była testowana
tol = 1000 # UWAGA, jeżeli ta wartość zostanie ustawiona zbyt nisko, może zostać wyliczony krótszy tor niż zadana liczba obrotów
...
def zadanie_1(iter_num,r0,v0,param,obroty):
    stop0 = time.time()
    #deklaracje prealokowane tablice dla polepszenia czasu wykonania programu
    ...
    narysuj(t_d,x_d,"x(t)_"+param,"t[s] x[m]")
    stop2 = time.time()
    print("Zadanie1 Drawing time: ",stop2-stop1,"s.")
# KONIEC KODU DO ZADANIA1
#################################################
# KOD DO ZADANIA2      
@njit
...
def zadanie_4(iter_num,r0,v0,param,obroty,tol,dt4):
    stop0 = time.time()
    #deklaracje tablic (preallokowane dla polepszenia czasu obliczeń)
    t  = np.linspace(0,iter_num,iter_num+1)
    x  = np.array(t)
    y  = np.array(t)
    vx = np.array(t)
...
#liczba iteracji celowo 2 razy większa niż okres komety Halleya(program napisano tak aby nie miało to wpływu na czas wykonania) 
iter_num = int(obroty*2*80*365*24*(3600/dt))
zadanie_1(iter_num,r0,v0,param,obroty)
zadanie_2(iter_num,r0,v0,param,obroty)
#liczba iteracji celowo 8 razy większa niż okres komety Halleya(program napisano tak aby nie miało to wpływu na czas wykonania)
