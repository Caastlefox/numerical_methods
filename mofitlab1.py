...
def fpsi(E,V,dx):
    ...
    for i in range(2,N+1):
        psi[i] = -2*m/(hbar**2)*(E-V[i])*(dx**2)*psi[i-1]-psi[i-2]+2*psi[i-1]
    return psi
...
for zero in mzero:
    ...
    plt.ylabel("psi",**csfont)
    plt.title("psi(x)",**csfont)
    for W in np.linspace(0,1000,26):
        V[int(N/2+1)] = -W/27211.6
        psiEzero = fpsi(zero,V,dx)
        C = Csqrt(psiEzero,dx)
        psiEzero /= C
        plt.scatter(xarr,psiEzero,marker=".",linewidths=0.1,label=str(W))
    plt.legend()
    plt.show()

for i in range(0,4):
    V[int(N/2+1)] = 500/27211.6
    psiEzero = fpsi(mzero[i],V,dx)
    C = Csqrt(psiEzero,dx)
    psiEzero /= C
    plt.scatter(xarr,psiEzero,marker=".",linewidths=0.1,label=str(W))